...
import NoAttempsLeft from "@/components/NoAttemptsLeft.vue";
...
const onSubmit = handleSubmit(async (values) => {
  if (values.value && values.value.length > 0) {
    ...
    try {
      ...
      emit('results', isCorrectGuess.value);
      resetForm();
    }
  }
});
...
<template>
  <section class="min-h-dvh p-5 max-w-screen-md my-0 mx-auto flex flex-col gap-10 items-center justify-evenly">
    <template v-if="store.attempts<3">
      <h1 class="text-center text-[clamp(22px,5vw,38px)] tracking-wide">Adivina el truco y obtén las coordenadas para ganarte un premio</h1>
